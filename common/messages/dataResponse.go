...
func (a *DataResponse) IsSameAs(b *DataResponse) bool {
	...
	defer messagesdataResponseIsSameAs.Observe(float64(time.Now().UnixNano() - callTime))	
		if b == nil {
		return false
	}
...
func (m *DataResponse) GetRepeatHash() interfaces.IHash {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseGetRepeatHash.Observe(float64(time.Now().UnixNano() - callTime))	
	return m.GetMsgHash()
}

func (m *DataResponse) GetHash() interfaces.IHash {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseGetHash.Observe(float64(time.Now().UnixNano() - callTime))	
	return m.GetMsgHash()
}

func (m *DataResponse) GetMsgHash() interfaces.IHash {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseGetMsgHash.Observe(float64(time.Now().UnixNano() - callTime))	
	if m.MsgHash == nil {
...
func (m *DataResponse) Type() byte {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseType.Observe(float64(time.Now().UnixNano() - callTime))	
...
func (m *DataResponse) Validate(state interfaces.IState) int {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseValidate.Observe(float64(time.Now().UnixNano() - callTime))	
...
func (m *DataResponse) LeaderExecute(state interfaces.IState) {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseLeaderExecute.Observe(float64(time.Now().UnixNano() - callTime))	
	m.FollowerExecute(state)
}

func (m *DataResponse) FollowerExecute(state interfaces.IState) {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseFollowerExecute.Observe(float64(time.Now().UnixNano() - callTime))	
...
func (e *DataResponse) JSONByte() ([]byte, error) {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseJSONByte.Observe(float64(time.Now().UnixNano() - callTime))	
...
func (m *DataResponse) UnmarshalBinaryData(data []byte) (newData []byte, err error) {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseUnmarshalBinaryData.Observe(float64(time.Now().UnixNano() - callTime))	
	defer func() {
		if r := recover(); r != nil {
...
func (m *DataResponse) UnmarshalBinary(data []byte) error {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseUnmarshalBinary.Observe(float64(time.Now().UnixNano() - callTime))	
...
func attemptEntryUnmarshal(data []byte) (entry interfaces.IEBEntry, err error) {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseattemptEntryUnmarshal.Observe(float64(time.Now().UnixNano() - callTime))	
	defer func() {
		if r := recover(); r != nil {
...
func attemptEBlockUnmarshal(data []byte) (eblock interfaces.IEntryBlock, err error) {
	callTime := time.Now().UnixNano()
	defer messagesdataResponse.Observe(float64(time.Now().UnixNano() - callTime))	
	defer func() {
		if r := recover(); r != nil {
...
func (m *DataResponse) MarshalBinary() ([]byte, error) {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseMarshalBinary.Observe(float64(time.Now().UnixNano() - callTime))	
...
func (m *DataResponse) String() string {
	callTime := time.Now().UnixNano()
	defer messagesdataResponseString.Observe(float64(time.Now().UnixNano() - callTime))	
